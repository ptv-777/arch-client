
# DICOM archive pipeline (pre-alpha)

Переходный монорепозиторий: сервер обработки, индексатор, упаковщик и
кроссплатформенный клиент. Код пока не готов для работы с реальными
медицинскими данными: нет аутентификации, аудита, миграций БД и завершённого
возобновляемого ingest-процесса.

Поэтапная архитектура, критерии готовности и открытые решения описаны в
[`docs/roadmap.md`](docs/roadmap.md).

## Быстрый старт (локально, без NAS)
1. Установить Python 3.10+ и создать venv:
   ```bash
   python -m venv .venv && source .venv/bin/activate
   pip install -r requirements.txt
   ```
2. Настроить переменные окружения (или `.env` — см. ниже):
   ```bash
   export DB_URL="sqlite:///dicom_index.sqlite3"
   export DICOM_ROOT="./_demo_studies"          # локальная папка с DICOM-файлами
   export CACHE_DIR="./cache"
   export INBOX_DIR="./_inbox_plus"
   ```
3. Инициализировать БД и прогнать индексацию демо-папки:
   ```bash
   python scripts/init_db.py
   python scripts/indexer.py --root "$DICOM_ROOT" --workers 8
   ```
4. Запустить API:
   ```bash
   uvicorn server.app:app --host 0.0.0.0 --port 8000
   ```
5. Запустить клиент:
   ```bash
   python -m client
   ```

## Переменные окружения
- `DB_URL` — строка подключения (SQLite / PostgreSQL / MySQL или MariaDB), например:
  - SQLite: `sqlite:///dicom_index.sqlite3`
  - PostgreSQL: `postgresql+psycopg2://user:pass@host:5432/dicom`
  - MySQL/MariaDB: `mysql+pymysql://user:pass@host:3306/dicom?charset=utf8mb4`
- `DICOM_ROOT` — корень канонического хранилища `/studies/...`
- `CACHE_DIR` — каталог кэша архивов (`TAR.zst`)
- `INBOX_DIR` — папка «+++» для ISO
- `RADIANT_CMD` — команда запуска просмотрщика (если нужно автозапускать)
- `CORS_ORIGINS` — список разрешённых browser-origin через запятую. По
  умолчанию CORS выключен; настольному клиенту он не нужен.

Создайте `.env` на основе `.env.example` или экспортируйте переменные.

### MySQL/MariaDB
1. Драйвер уже в `requirements.txt` (`pymysql`). При использовании системного дистрибутива достаточно `pip install -r requirements.txt`.
2. Укажите строку подключения в `DB_URL`, пример: `mysql+pymysql://user:pass@host:3306/dicom?charset=utf8mb4` (или `mariadb+pymysql://...`).
3. Проинициализируйте схему под новую БД: `python scripts/init_db.py`, затем переиндексируйте каталог.
4. Параметр `pool_pre_ping=True` в коде уже настроен, чтобы MySQL-пулы переживали обрывы соединений.

## Эндпоинты (MVP)
- `GET /search?name=Иванов Иван&dob=19790101&sex=M&year=2024` → исследования (по `StudyInstanceUID`) с объёмами и количеством файлов.
- `GET /package?study_uid=<UID>` → формирование и выдача архива (если кэш есть — отдаёт сразу).

## Что уже есть
- Индексатор: читает заголовки DICOM (`stop_before_pixels=True`), сохраняет метаданные и пути.
- API FastAPI: поиск и атомарная упаковка в `TAR.zst`; структура
  `исследование/серия/SOPInstanceUID.dcm` не допускает потери одноимённых
  файлов из разных серий.
- Клиент PySide6: поиск → выбор → скачивание → безопасная распаковка → запуск
  настроенного просмотрщика. Поддерживаются `tar.zst`, ZIP/TAR и ISO.
- Скелет ISO-инжеста + systemd шаблоны.
- Схема БД через SQLAlchemy (можно переключить на PostgreSQL).

## Что осталось доделать (после MVP)
- Продолжение `.part` в клиенте и проверка контрольной суммы; серверная выдача
  пакета уже поддерживает HTTP Range.
- Полный ISO-инжест (монтаж ISO, парсинг DICOMDIR) под вашу ОС.
- Дедуп по `SOPInstanceUID` + `pixeldata sha256`.
- Возобновляемый журнал ingest, карантин и явная обработка коллизий.
- Альтернативные имена пациента и русское ФИО из входной папки.
- RBAC, аудит, TLS, лимиты скорости и упаковка клиента под целевые ОС.
- Политика лимита и очистки локального кэша клиента.

## Проверка

```bash
python -m pip install -r requirements-test.txt
python -m unittest discover -s tests -v
```

Тесты проверяют сохранение одноимённых DICOM-файлов из разных серий,
ограничение выдачи каталогом `DICOM_ROOT` и защиту распаковки от выхода за
выбранную папку.

## Структура
```
server/
  app.py         # FastAPI, эндпоинты
  config.py      # загрузка env
  db.py          # модели и сессия SQLAlchemy
  packager.py    # TAR.zst упаковщик и кэш
  utils.py       # нормализация имени и т.п.
  ingest/
    ingest.py    # скелет ISO-инжеста
    systemd/
      iso-import@.service
      iso-watch.path
client/
  client.py      # GUI PySide6
  archive.py     # безопасная распаковка пакетов
  config.py
  files.py       # безопасные локальные имена файлов
scripts/
  init_db.py     # создание таблиц
  indexer.py     # индексация каталога DICOM
.env.example
requirements.txt
requirements-test.txt
README.md
```

## Лицензия
MIT (настраиваемо).
# arch-client
